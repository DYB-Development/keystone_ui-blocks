...
module KsBlocks
  module Layout
    ...
    class_methods do
      def block_layout(column, kind: :blocks, columns: Grid::COLUMNS, row_height: Grid::ROW_HEIGHT, gap: Grid::GAP, narrow_columns: nil, narrow_row_height: nil)
        ...
        define_method(:layout_data) do
          KsBlocks.layout_data(public_send(column), kind: block_layout_kind, grid: { columns: columns, row_height: row_height, gap: gap, narrow_columns: narrow_columns || columns, narrow_row_height: narrow_row_height || row_height }, offered: block_layout_offered)
        end
      end
    end
  end
